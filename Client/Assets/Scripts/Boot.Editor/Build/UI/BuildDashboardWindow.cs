...
namespace Boot.Editor.Build
{
    ...
    public sealed class BuildDashboardWindow : OdinMenuEditorWindow
    {
        ...
        public sealed class BuildView
        {
            ...
            [ShowInInspector, LabelText("打包后发布到 CDN")]
            [Tooltip("勾选后，一键打包成功后自动把热更资源发布到 Server/Res/CDN（相对仓库根 KJ）。Host 模式设备将从 CDN 下载最新资源。")]
            public bool PublishToCdn
            {
                get => _window._profile != null && _window._profile.PublishToCdn;
                set
                {
                    if (_window._profile != null)
                        _window._profile.PublishToCdn = value;
                }
            }
