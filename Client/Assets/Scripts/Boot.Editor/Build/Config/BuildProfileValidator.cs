...
namespace Boot.Editor.Build
{
    ...
    public static class BuildProfileValidator
    {
        public static List<BuildIssue> Validate(BuildProfile profile)
        {
            ...
            {
                if (string.IsNullOrWhiteSpace(profile.CdnServerRoot))
                    issues.Add(BuildIssue.Error(
                        BuildErrorCodes.PlanInvalidProfile, "P0.Plan",
                        "PublishToCdn is enabled but CdnServerRoot is empty"));
                else if (profile.AssetMode != Framework.Asset.AssetConfig.PlayMode.Host)
                    issues.Add(BuildIssue.Warning(
                        BuildErrorCodes.PlanInvalidProfile, "P0.Plan",
                        "PublishToCdn is enabled but AssetMode is not Host; devices won't query the CDN"));
            }
