...
namespace Boot.Editor.Build
{
    ...
    public class BuildProfile : ScriptableObject
    {
        ...
        [Header("CDN 发布")]
        [Tooltip("构建成功后自动发布热更资源到 CDN。用于 Host 模式设备从 CDN 下载最新资源。")]
        public bool PublishToCdn = false;

        [Tooltip("CDN 目标根目录（相对仓库根 KJ，默认 Server/Res/CDN/Android/DefaultPackage）。构建成功后把 P4 的 YooAsset 输出复制到这里。")]
        public string CdnServerRoot = "Server/Res/CDN/Android/DefaultPackage";

        [Tooltip("发布到 CDN 的版本目录名（为空则使用 Profile.VersionName）。")]
        public string CdnPublishVersion = "";
        ...
        /// <summary>
        /// 仓库根目录（KJ）——Unity 工程（Client）的父目录。
        /// 统一用相对路径推导，Server、配置表等仓库级内容都基于此解析。
        /// </summary>
        ...
        /// <summary>
        /// CDN 目标根目录（绝对路径）。<see cref="CdnServerRoot"/> 相对仓库根解析，
        /// 也支持绝对路径。构建成功后 P10_PublishCdnStage 会把 P4 的 YooAsset 输出复制到这里。
        /// </summary>
        public string GetCdnRoot()
        {
            if (string.IsNullOrWhiteSpace(CdnServerRoot))
                return null;
            if (Path.IsPathRooted(CdnServerRoot))
                return Path.GetFullPath(CdnServerRoot);
            return Path.GetFullPath(Path.Combine(GetRepoRoot(), CdnServerRoot));
        }
