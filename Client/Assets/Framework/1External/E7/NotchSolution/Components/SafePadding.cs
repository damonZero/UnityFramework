...
namespace E7.NotchSolution
{
    ...
    {
        [SerializeField, Tooltip("是否是根安全区域")] public bool IsMainSafePadding;

        public static SafePadding Instance{ get; private set; }

        Rect safeArea;

        public Vector4 FinalPaddingsLdur { get; private set; }

        protected override void Awake()
        {
            base.Awake();
            if (!IsMainSafePadding) return;
            Instance = this;
        }
        ...
        protected override void UpdateRect()
        {
            var selectedOrientation =
                orientationType == SupportedOrientations.Dual
                    ? NotchSolutionUtility.GetCurrentOrientation() == ScreenOrientation.LandscapeLeft
                        ? landscapePaddings
            ...
            FinalPaddingsLdur = finalPaddingsLDUR;
        }
#pragma warning disable 0649
        [SerializeField] private SupportedOrientations orientationType;

        [SerializeField] private PerEdgeEvaluationModes portraitOrDefaultPaddings = new PerEdgeEvaluationModes();

        [SerializeField] private PerEdgeEvaluationModes landscapePaddings = new PerEdgeEvaluationModes();
