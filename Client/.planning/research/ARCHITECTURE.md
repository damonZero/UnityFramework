...
**研究日期:** 2026-06-26 / 实际实现更新: 2026-07-08
...
> - **`ObjectPoolManager` → `Framework/Pool/ObjectPool<T>` + `Framework/Pool/Unity/GameObjectPool` + `Framework/Cache/BoundedStore<TKey,TValue>` + `Core/Pool/PoolService` (DI 桥接)**。§7 的 IModule ObjectPoolManager 方案已被上述替代。
...
> - **构建打包管线** 不在本文档覆盖范围内，详见 `ProgressDoc/Discuss/资源系统/Hy3_构建打包全流程管线_需求分析与设计.md` 及代码 `Assets/Scripts/Boot.Editor/Build/`（BuildProfile-only P0-P9 + `BuildPipelineRunner` + Dashboard）。
...
> **⚠️ 本章节为设计草稿**。实际实现已大幅偏离：
> - `ObjectPoolManager : IModule` → 已被 `Framework/Pool/` 下的 `ObjectPool<T>` + `GameObjectPool` + `PoolService` (DI 桥接) 替代。
> - 缓存策略：旧 `Cache` → `BoundedStore<TKey,TValue>` + `IStoreEvictionPolicy`（`LruPolicy`/`TtlPolicy`/`CapacityPolicy`/`CompositePolicy`）。
> - `GameObjectPool`：五字典合并 `PrefabPoolState` + 反向索引污染检测 + 实例库存策略 `IInstanceRecyclePolicy`。
> - 详见 `ProgressDoc/Review/对象池与缓存系统_代码审查_2026-07-08.md`。
...
| 对象池 | 高 | ✅ 已实现——Framework/Pool + BoundedStore 策略体系，82/84 单测通过 |
| 构建管线 | 高 | ✅ 已实现——BuildProfile-only P0-P9 + SHA-256 内容指纹 + HybridCLR 六子步骤缓存 + Dashboard |
...
*研究完成日期: 2026-06-26 / 对齐更新: 2026-07-08*
