...
> **⚠️ 状态对齐（2026-07-17 补）**：当前代码已收敛为 **BuildProfile-only + BuildPipelineRunner + P0-P9 + fingerprint** 架构。旧 `BuildConfig.cs` / `BuildConfig.asset` / `BuildReport.cs` / `StageDependencyTracker.cs`、`.markers` 续跑和 mask 手动 Stage 执行已删除。本文保留历史需求分析与演进讨论；涉及 BuildConfig、旧 S0-S9 marker/mask 的段落仅作为历史背景，不再代表当前实现。人工构建唯一菜单入口是 `KJ/Build/Dashboard`；CI 使用 `Boot.Editor.Build.BuildCommandLine.Run -profile <BuildProfile.asset>`。
>
...
该文件提供一组开发内循环入口和供复合流程调用的内部步骤，关键链路如下：

| 入口/步骤 | 可见性 | 做的事 | 产出 |
| --- | --- | --- | --- |
| `Prepare Runtime Assets And Boot` | 开发菜单 | 日常编译同步 → EditorSimulate 包 → Boot 场景 → 校验 | 可直接 Editor Play 的运行资产 |
| `Generate All Sync And Prepare Boot` | 开发菜单 | `GenerateAll` 后执行完整 Editor 准备闭环 | 结构变化后的完整运行资产 |
| `Generate Runtime Assets And Sync` | 开发菜单 | 安装运行时 → `CompileDll` → `GenerateAotMetadataIfNeeded`（strip AOT）→ 同步 | 热更 DLL + AOT metadata DLL 同步进资产目录 |
| `Prepare YooAsset Editor Simulate Package` | 开发菜单 | `EditorSimulateBuildInvoker.Build(...)` → 写 `AssetConfig.EditorSimulatePackageRoot` | **仅 EditorSimulate 包**（供 Editor Play Mode 用） |
| `Install HybridCLR Runtime` | Maintenance 菜单 | 安装/校验 HybridCLR 运行时 | `HybridCLR` 运行时就位 |
| `Prepare Boot Scene` | Maintenance 菜单 | 打开 Boot 场景 → Apply → 确保进 BuildSettings → 保存 | 场景就位 |
| `Validate Outputs` | Maintenance 菜单 | 检查同步的 DLL 都在、无多余 DLL | 静态校验通过/抛异常 |
| `GenerateAllAndSync` / `SyncExistingOutputs` / `ApplyToOpenEntry` | 内部步骤 | 供上述复合流程调用，不单独暴露菜单 | 中间状态 |
...
> 
> 补充约束 [REVIEW F6]：
> - **S4（真包落到 StreamingAssets）→ S6**：包必须先进 `Assets/StreamingAssets/yoo/DefaultPackage` 才能被 BuildPlayer 嵌入 Player。
...
- **调用 YooAsset 真实生产构建 API**（**不是** `EditorSimulateBuildInvoker`）。当前 `DefaultPackage` 仅含 `PackRawFile` 收集器，使用 `RawFileBuildPipeline + EBundleType.RawBundle`；若未来加入 AssetBundle 内容，应拆分 Package 或重新评审包级 pipeline 类型：
  - BuildTarget = `BuildConfig.Platform`
  - PackageName = `DefaultPackage`
  - **Offline 模式**：输出到 `Assets/StreamingAssets/yoo/DefaultPackage/`（根目录取 `BundleBuilderHelper.GetStreamingAssetsRoot()`；需随后 `AssetDatabase.Refresh`）
...
- **S4 前置清理**：清空 `Assets/StreamingAssets/yoo/DefaultPackage/` 旧产物，防 YooAsset 缓存命中导致用旧 bundle（类似旧项目的 `CleanStreamCacheTask`）。
...
- 不变量：`Assets/StreamingAssets/yoo/DefaultPackage/DefaultPackage.version` 与版本 hash 存在；`hotupdate` bundle 文件存在且体积 > 0；包内 `Dlls/` 含 10 个 `.dll.bytes`、`AotMetadata/` 含 3 个（用 YooAsset 清单或解包校验）。
...
- YooAsset 包：`Assets/StreamingAssets/yoo/DefaultPackage/DefaultPackage.version` 与版本 hash 存在；`hotupdate` bundle 含 10+3 个 `.dll.bytes`（**校验 StreamingAssets，不只是 Bundles/**）。
...
1. **单一编排器 `KJBuildPipeline.Build(BuildConfig)`，而非再加一堆散装 MenuItem。**
   HybridCLR 菜单保留为开发内循环；人工完整构建统一从 `KJ/Build/Dashboard` 发起，无头入口为 `KJ.Build.KJBuildPipeline.BuildFromCommandLine`（供 CI `-executeMethod`）。
...
| YooAsset 离线包输出位置不匹配 builtin 读取 | Offline 模式 Player 找不到包 | S4 使用 `BundleBuilderHelper.GetStreamingAssetsRoot()`，当前输出到 `Assets/StreamingAssets/yoo/DefaultPackage` |
...
- **M2 Stage 4（YooAsset 正式包）** ✅：当前纯 RawFile `DefaultPackage` 使用 `RawFileBuildPipeline + RawBundle`，产出到 `Assets/StreamingAssets/yoo/DefaultPackage`；`ClearBuildCacheFiles = true` 关闭增量缓存避免旧残留。
...
- **保留**：`KJ/HybridCLR/*` 的四个开发入口；安装、Boot 场景修复和输出校验收入 `Maintenance` 子菜单；同步与 Entry 写入子步骤不再单独暴露。
- **人工入口**：`KJ/Build/Dashboard`；CI 继续使用 `KJ.Build.KJBuildPipeline.BuildFromCommandLine`。
...
1. ✅ 从 `KJ/Build/Dashboard` 执行完整构建，可在 **Android IL2CPP** 产出 Player + `Assets/StreamingAssets/yoo/DefaultPackage/` 真包（Offline 模式，builtin 文件系统可直接加载）。
...
| S4 BuildYooAsset | 纯 RawFile 包用 RawFileBuildPipeline 打真包到 StreamingAssets。 | 按 Package 内容选择正确 pipeline/type；输出 YooAsset manifest/hash/size；校验 collector tag、RawFile 规则和 manifest `RawBundle` 类型；保留构建产物到 `BuildInfo/BaseBundle` 或 v2 归档。 |
...
### F.14 AOP-PERF-01 打包耗时监控雏形（2026-07-19）

首版采用显式 Observability，不使用 Source Generator 或 ILPostProcessor 修改方法体：

- 新增 Editor-only、`autoReferenced=false` 的 `Assets/Framework/Aop/Aop.asmdef`，提供纯 C# `AopRuntime`、`AopSpan`、`IAopClock`、`IAopCollector` 和有界 `InMemoryAopCollector`；当前不进入 Player，Runtime 开放留到 HybridCLR 归属评审后；
- Span 使用 `Stopwatch.GetTimestamp()` 单调时钟，记录 run/span/parent、类别、耗时、线程、状态和异常类型，不记录异常消息或业务参数；
- `BuildPipelineRunner` 负责 session 生命周期，将性能明细写入 `BuildReportData.PerformanceSpans`，报告 schema 升至 1.1.0；
- P2 监控 GenerateAll；P3 监控清理、CompileDll、资源同步和 Refresh；P4 监控 StreamingAssets 清理、YooAsset package build 和 Refresh；P6 监控 Refresh 和 BuildPlayer；
- `BuildTelemetry.Measure` 保持原异常继续抛出，Collector 异常被隔离并计入 `PerformanceCollectorFailureCount`；
- Markdown 报告按耗时降序展示 Operation/Category/Status/Duration/Exception，JSON 保存完整结构化 Span；
- Unity 2022.3.62f2 batchmode 编译通过，`Boot.Editor.Build.Tests` 14/14 全绿，其中 5 个新增用例覆盖单调耗时/父子关系、异常脱敏、Collector 故障隔离、容量丢弃和报告 JSON 序列化。

该雏形尚未实现性能基线、预算告警、Chrome Trace、指标值和透明 IL 织入。下一验收步骤是在 Standalone P0-P9 完整构建中检查真实耗时分布和报告完整性。

### F.15 Android P0-P9 首次真实闭环修复（2026-07-19）

- P4/P7 以 `BundleBuilderHelper.GetStreamingAssetsRoot()` 为唯一 builtin 根，修复旧硬编码漏掉 `yoo/` 的误报；
- P3 保留 P2 生成的裁剪 AOT 目录，并复用 `KJHybridClrBuildTools.SyncExistingOutputs()`，精确同步当前平台声明的 10 个热更 DLL 与 3 个 AOT metadata；
- P6 事务化关闭并恢复 `exportAsGoogleAndroidProject`，清理同名旧导出目录后生成真实 APK；
- fingerprint 使用 `IBuildStage.Version`，且本轮执行的产物/事务依赖会级联禁止下游跳过，避免旧 YooAsset 包或 APK 假阳性；
- Dashboard 最终 RunId `20260719_131655_9b8295750`：仅 P2 合法跳过，P3-P9 实际执行并通过，生成 APK 74,653,754 bytes，`build_report.json` 为 `AllPassed=true`。
