...
description: >
  ...
  核心规则：事件必须是 readonly struct；[GameEvent] 只能标记 struct；Framework.Event 零外部依赖，不直接绑定 MessagePipe；实际 MessageBroker 注册在 Core 层 ArchitectureContainerRegistration 中通过反射完成。
metadata:
  doc: CODEMAP.md
  layer: Framework
---
...
Core.Architecture.ArchitectureContainerRegistration (注册层)
...
```
ArchitectureContainerRegistration.RegisterGameEvents()
  │
...
| 事件 | 命名空间 | 发布者 | 时机 |
|------|----------|--------|------|
| `AppStartedEvent` | `Core.Architecture` | `SystemManager.InitAll()` | 所有 Core 系统初始化完成 |
| `AppShuttingDownEvent` | `Core.Architecture` | `SystemManager.ShutdownAll()` | 系统即将关闭 |
...
5. **新事件放对层级** — Core 事件放 `Core.Architecture.Events/`，业务事件放 `General/Events/` 或 `Project/<Feature>/Data/`
6. **不跨层依赖事件类型** — Project 可以引用 Core 的事件，但不要反过来

## 扩展

如需自定义事件后端（不用 MessagePipe），只需修改 `ArchitectureContainerRegistration.RegisterGameEvents()` 和 `GeneralContainerRegistration.RegisterBusinessEvents()` 中的注册逻辑。`GameEventAttribute` 和 `GameEventTypeScanner` 不需要改。
