...
KJ — Unity 2022.3.62f2 客户端游戏框架。VContainer + MessagePipe + YooAsset + HybridCLR。（Luban 配置表尚在规划阶段，详见 ROADMAP。）
...
> **改文档/规则/技能前必读** `.claude/rules/agentscfg-workflow.md`：只改 `.agentscfg/` 源（`instructions/BASE.md`、`targets/claude/rules/*.md`、`skills/*`），不要直接改 `CLAUDE.md` / `AGENTS.md` / `.claude/rules/` 等生成产物（会被 `npx agentscfg` 同步覆盖）。

**遇到运行期问题/调试分析时**，优先读取 `Logs/Runtime/latest.session.json` 和 `Logs/Runtime/latest.jsonl`，按 phase/module 回溯启动链路。不得依赖用户截图 Console。
...
- **Framework**：KJ 自有的独立包（`Asset/`、`Event/`、`Pool/`、`Cache/`、`BuildPipeline/`）。**最多只能依赖 Packages**，不能引用 Scripts 下任何代码。稳定底层模块直接放在 `Assets/Framework/`，不使用 `Assets/Framework/Package/`。`BuildPipeline/` 是纯数据契约层（`noEngineReferences=false`），不引用 `UnityEditor`/`Boot`/`Core` 等业务层。如果需要项目能力，通过接口、适配器或静态委托注入，由 Core 层桥接。**所有 Framework 模块推荐引用 ZString + ZLinq**（零依赖高性能基础库，按需使用不强制），详见 `.claude/rules/zero-allocation-libs.md`。
...
- **Core**：引擎基础设施。引用 `Asset / Event / Pool / Cache / Log / RuntimeLog / ZLinq / ZLogger.Unity / AssetShared / VContainer / MessagePipe / MessagePipe.VContainer / UniTask`（当前不引用 Boot/Launcher，靠 VContainer 反向注入）。不引用 General/Project。
...
- **19 个热更程序集**：`Boot, Core, General, Project, Pool, Cache, Event, Asset, Log, RuntimeLog, Timer, Framework.ViewCache, Framework.DependencyInjection, Framework.View, Framework.MVVM, Framework.View.Navigation, Framework.Restart, Framework.Coverage, Framework.Touch`（事实源 `ProjectSettings/HybridCLRSettings.asset` 的 `hotUpdateAssemblies`；新增程序集只改 asset 与对应 asmdef，测试动态校验）。
...
- **ZString** — 热路径字符串拼接时使用（单字面量/单变量不需要）
- **ZLinq** — 有 LINQ 操作时替代 `System.Linq`（原本没用 LINQ 就别硬写链式）
- **ZLogger** — Core 层通过 `GameLogBridge` 桥接；`[ZLoggerMessage]` 用于 Core/General/Project 的 DI 日志
- **Pool / Cache** — Runtime 代码临时集合必须走 `CollectionPool.Rent` + `using`，频繁创建对象走 `ObjectPool<T>`，有界缓存走 `BoundedStore`
- **日志写入** — 三层 API（Framework/Boot 用 `GameLog`，Core+ 用 `ILogger<T>` + `[ZLoggerMessage]`）；module 不可省略，异常必须传原始 Exception；AI 分析优先读 `Logs/Runtime/latest.jsonl`；详见 `.planning/AI_RUNTIME_LOGGING.md`
- **启动最小层白名单**：
  - Launcher：`UniTask, YooAsset, HybridCLR.Runtime, AssetShared` + 可选 `ZString`
  - Boot：`Asset, Log, RuntimeLog, UniTask, AssetShared, YooAsset, Launcher` + 可选 `ZString, ZLinq`
- **不要为用而用** — 没有需求就别硬上
...
| `Framework/` | `Framework.Asset`、`Framework.Event`、`Framework.Pool`、`Framework.Cache`、`Framework.Log`、`Framework.RuntimeLog`、`Framework.BuildPipeline`、`Framework.Aop` 等 |
...
- **静态变量与软重启**：软重启（`Boot.GameLife.GameRestart.SoftRestart`）会反射把「可变 static 字段」重置为 default。写静态字段必须遵守「不变值用 `const`/`static readonly`；可变 static 声明初始值必须是 default；非默认起始值用 `[SoftRestartField(initialValue: x)]`」约定，否则 `static bool N = true` 会在重启后静默变 false，详见 `.claude/rules/static-restart-state.md`。
